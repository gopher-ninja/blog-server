...
func (bc *BlogCreate) SetBlogId(u uuid.UUID) *BlogCreate {
	bc.mutation.SetBlogId(u)
	return bc
}

// SetBlogTitle sets the "blogTitle" field.
func (bc *BlogCreate) SetBlogTitle(s string) *BlogCreate {
...
func (bc *BlogCreate) check() error {
	if _, ok := bc.mutation.BlogId(); !ok {
		return &ValidationError{Name: "blogId", err: errors.New(`ent: missing required field "Blog.blogId"`)}
	}
	if _, ok := bc.mutation.BlogTitle(); !ok {
		return &ValidationError{Name: "blogTitle", err: errors.New(`ent: missing required field "Blog.blogTitle"`)}
	}
...
func (bc *BlogCreate) createSpec() (*Blog, *sqlgraph.CreateSpec) {
	...
	if value, ok := bc.mutation.BlogId(); ok {
		_spec.SetField(blog.FieldBlogId, field.TypeUUID, value)
		_node.BlogId = value
	}
	if value, ok := bc.mutation.BlogTitle(); ok {
		_spec.SetField(blog.FieldBlogTitle, field.TypeString, value)
